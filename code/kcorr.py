...
def main(args):
    ...
    if args.type == "S":
        kcorr_df = pd.read_csv(
            os.path.join(ROOT, "data/kcorr_sgrbs.csv"), index_col="GRB"
        )
    elif args.type == "L":
        kcorr_df = pd.read_csv(
            os.path.join(ROOT, "data/kcorr_lgrbs.csv"), index_col="GRB"
        )
    else:
        print(
            "Please provide a valid type argument.\n"
    ...
    kcorr_df["dl_cm"] = cosmo.luminosity_distance(kcorr_df["z"]).value * 3.08568e24
    ...
    for grb in grbs:
        ...
        if args.type == "S":
            ...
            outfile = os.path.join(ROOT, "data", "SGRBS", grb, "".join([grb, "_k.csv"]))
        elif args.type == "L":
            infile = os.path.join(ROOT, "data", "LGRBS", grb, "".join([grb, ".csv"]))
            outfile = os.path.join(ROOT, "data", "LGRBS", grb, "".join([grb, "_k.csv"]))
        else:
            print(
                "Please provide a valid type argument.\n"
                "The type of GRB: L - long, S - short"
        ...
        k_data["Lum50err"] = gmean(
            [k_data["Lum50pos"].values, np.abs(k_data["Lum50neg"].values)]
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="A script to k-correct the GRB" " samples."
    )
    parser.add_argument(
        "-t", "--type", required=True, help="The type of GRB: L - long, S - short"
    )
