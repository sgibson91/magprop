...
for i in range(len(po_Nacc)):
    if po_beta[i] > 0.27:
        ...
        if po_Rm[i] >= R:
            po_Nacc[i] = ((GM * po_Rm[i]) ** 0.5) * (po_Mdotacc[i] - po_Mdotprop[i])
        else:
...
po_Lprop = (-1.0 * po_Nacc * po_omega) - ((GM / po_Rm) * po_eta2 * (po_Mdisc / tvisc))
...
b_Lprop = (-1.0 * b_Nacc * b_omega) - ((GM / b_Rm) * b_eta2 * (b_Mdisc / tvisc))
...
ax3.set_ylabel(
    "Dipole Luminosity ($10^{50}$ ${\\rm erg}$ ${\\rm s}^{-1}$)", fontsize=12
)
...
ax4.set_ylabel("Total Luminosity ($10^{50}$ ${\\rm erg}$ ${\\rm s}^{-1}$)", fontsize=12)
